...
import { skillHasRunnableToolIds } from '../../../skill/skill-runnable.util';
import {
...
export class RequestedSkillRunService {
  ...
  /**
   * C 端发消息前：角色可见 + Skill Tool 与用户允许 Tool 有交集（与进入 run 条件一致）。
   */
  ...
  async loadRunContext(input: {
    ...
    const { skill, skillTools } = await this.resolveRunnable(input);
    return {
      skillId: skill.id,
      skill,
  ...
  private async resolveRunnable(input: {
    ...
    if (skillTools.length === 0) {
      throw new RequestedSkillRunError(
        'SKILL_TOOLS_EMPTY',
        `skill ${input.skillId} has no usable tools for the current user or agent binding`,
      );
    }
  ...
  private pickSkillToolsFromAllowed(
    ...
    if (!skillHasRunnableToolIds(skill, allowedToolIds)) {
      return [];
    }
