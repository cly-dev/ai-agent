...
function parseAgentId(): number | null {
  const raw = process.env.SEED_SKILL_AGENT_ID?.trim();
  if (!raw) {
    return null;
  }
  const id = Number(raw);
...
function parseAppClientId(): number {
  const raw = process.env.SEED_SKILL_APP_CLIENT_ID?.trim() ?? '2';
  const id = Number(raw);
...
async function resolveAgent(appClientId: number): Promise<{
  id: number;
  appClientId: number;
  name: string;
}> {
  ...
  if (explicitAgentId != null) {
    ...
    if (!agent) {
      throw new Error(
        `agent id=${explicitAgentId} not found under appClientId=${appClientId}`,
      );
    }
    return agent;
  }
  ...
  if (!anyAgent) {
    throw new Error(
      `no agent under appClientId=${appClientId}; run prisma/scripts/seed-ops-agent.ts first`,
    );
  }
  return anyAgent;
}

async function pickAgentToolIds(agentId: number, maxTools: number): Promise<number[]> {
