...
| **readiness** | plan 之后：消息质量、smalltalk、gather 步槽位（CP7 LLM）；入口先 `applySkillFrameContext` 展开外层 `kind=skill` 步 |
...
|------|--------|----------------|
| `smalltalk` | intent / readiness | `AGENT_SUMMARIZE_SMALLTALK` |
| `message_unclear` | intent / readiness | `direct_user` + clarification guidance |
| `unsupported_scope` | intent / readiness / llm 兜底 | `direct_user` + unsupported guidance |
| `intent_recall_failed` | intent | 同上 |
| `clarification` | readiness（CP7） | `AGENT_RESPOND_CLARIFICATION` |
...
| CP3 | `!isUserIntentClear(message)` | respond → `message_unclear` | intent 未跑 smalltalk 拦截后的兜底 |
| CP4 | `detectIntentKind === smalltalk` | respond → `smalltalk` | 同上 |
| CP5 | pending 为 **tool gather** 且 `scopedTools.length === 0` | respond → `unsupported_scope` | skill 帧 bind 后无工具等 |
| CP6 | 无 plan / pending 为 **plan answer** / 非 tool gather / obs 已满足（并 **推进 plan**）/ 无 businessFields | ready 或 `pendingRespond` | 常见放行 |
| CP7 | LLM 判定缺 businessFields | respond → `clarification` | 有 gather 步且槽位未齐 |

**与 intent 的分工**

- **intent 路径**：CP3/CP4 在 intent 已处理；能到 readiness 的通常只剩 CP5–CP7。
- 所有路径均经 **intent → plan**；smalltalk / 意图不清在 intent 早期退出。
...
- `AGENT_READINESS_SLOT_LLM=0` → 跳过 CP7，直接 ready（由后续 llm/工具环处理缺参）。

## 典型路径示例

```text
# 缺 productId（外层 skill 已展开为内层 gather）
intent → plan → readiness(CP7) → summarize(clarification)
...
| `AGENT_READINESS_SLOT_LLM` | 开启 | 关闭后 CP7 不跑 LLM |

## Prompt 模板

| Key | 用途 |
|-----|------|
| `agent.readiness_slot_check` | CP7 结构化 `{ ready, missingFields[] }` |
...
| CP7 关闭 / LLM 失败 | 不澄清，进入 llm；可能由工具校验或 resultCheck 再兜底 |
| `plan` 被 skip（`tools_disabled`） | `taskPlan` 为 null，readiness 放行 → llm；依赖 `enableToolCall` 关闭工具环 |
| intent 路径 CP3/CP4 | 与 intent 重复判定，无害 |

## 审查记录（逻辑要点）

### `observation_satisfied` 时推进 plan

仅在 **本 run** `toolObservations` 满足当前 gather 步时，readiness 返回 `observation_satisfied`。**GOA 预载永远不能令 gather 跳步。** 写步永不由此路径满足。Plan 推进（L1 `plan_sync`）在 **`llm` 入口**统一执行，readiness 只做判定。`evaluateTurnReadiness` 要求传入 `observationBuckets`（`planObservationBucketsFromState`）。
...
`plan → readiness(clarification|smalltalk) → summarize` 时，若 `taskPlan` 仍有 pending gather 步，`shouldContinuePlanAfterSummarize` 曾为 true，导致同轮 `summarize → llm`。  
...
| 文件 | 说明 |
|------|------|
| `engine/turn/turn-readiness.util.ts` | CP 评估主逻辑 |
| `engine/turn/turn-readiness-llm.util.ts` | CP7 LLM |
