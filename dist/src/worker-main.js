...
const worker_health_server_util_1 = require("./bootstrap/worker-health-server.util");
function applyWorkerProcessDefaults() {
...
async function bootstrap() {
    ...
    (0, worker_health_server_util_1.startWorkerHealthServer)();
}
(0, register_process_error_handlers_util_1.registerProcessErrorHandlers)();
bootstrap().catch((error) => {
    var _a;
    if (error instanceof Error) {
        common_1.Logger.error('Worker bootstrap failed', (_a = error.stack) !== null && _a !== void 0 ? _a : error.message);
    }
    else {
        common_1.Logger.error('Worker bootstrap failed', String(error));
    }
    process.exit(1);
});
