...
let LlmService = LlmService_1 = class LlmService {
    ...
    async resolveInvocationMaxTokens(messages, hints) {
        var _a, _b;
        const config = await this.getCachedConfig();
        const parameters = this.normalizeParameters(config.parameters);
        const contextLength = this.resolveContextLength(parameters);
        ...
        return this.capOutputMaxTokens(targetOutput, contextLength, inputTokens, (hints === null || hints === void 0 ? void 0 : hints.callKind) === 'summarize'
            ? this.readSummarizeOutputTokenReserve()
            : undefined);
    }
    async getMessageTokenBudget(hints) {
        ...
        if (contextLength != null && contextLength > outputReserve) {
            return Math.max(512, contextLength -
                outputReserve -
    ...
    async fitMessagesToBudget(messages, hints, budgetOverride) {
        const budget = budgetOverride !== null && budgetOverride !== void 0 ? budgetOverride : (await this.getMessageTokenBudget(hints));
    ...
    async createLangChainChatModelForMessages(messages, options) {
        ...
        const resolvedMaxTokens = (_a = options === null || options === void 0 ? void 0 : options.maxTokens) !== null && _a !== void 0 ? _a : (await this.resolveInvocationMaxTokens(fitted.messages, options === null || options === void 0 ? void 0 : options.budgetHints));
    ...
    async invokeWithLangChain(input, forceStreaming, handlers) {
        var _a, _b;
        const invocationMaxTokens = await this.resolveInvocationMaxTokens(input.messages, input.budgetHints);
        const model = await this.createLangChainChatModel({
    ...
    readSummarizeOutputTokenReserve() {
        var _a;
        const raw = (_a = process.env.LLM_SUMMARIZE_OUTPUT_TOKEN_RESERVE) === null || _a === void 0 ? void 0 : _a.trim();
        if (!raw) {
            return LlmService_1.DEFAULT_SUMMARIZE_OUTPUT_TOKEN_RESERVE;
        }
        const parsed = Number.parseInt(raw, 10);
        return Number.isFinite(parsed) && parsed >= 512
            ? parsed
            : LlmService_1.DEFAULT_SUMMARIZE_OUTPUT_TOKEN_RESERVE;
    }
    async resolveOutputTokenReserve(hints) {
        const configuredOutput = await this.getResolvedMaxTokens();
        return this.resolveOutputTokenTarget(configuredOutput, hints);
    }
    resolveOutputTokenTarget(configuredOutput, hints) {
        if ((hints === null || hints === void 0 ? void 0 : hints.callKind) === 'summarize') {
            return Math.max(configuredOutput, this.readSummarizeOutputTokenReserve());
        }
        return configuredOutput;
    }
    capOutputMaxTokens(configuredOutput, contextLength, inputTokens, minOutputFloor) {
        if (contextLength == null) {
            return configuredOutput;
        }
        ...
        if (available < configuredOutput) {
            if (minOutputFloor != null && available < minOutputFloor) {
                this.logger.warn(`llm summarize output squeezed: available=${available} target=${configuredOutput} reserve=${minOutputFloor} inputTokens=${inputTokens} contextLength=${contextLength}`);
            }
...
LlmService.DEFAULT_SUMMARIZE_OUTPUT_TOKEN_RESERVE = 2048;
