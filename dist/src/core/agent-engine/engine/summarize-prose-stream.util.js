...
exports.resolveSummarizeLlmProseForStorage = exports.finalizeSummarizeProseStreamAfterLlm = exports.createSummarizeProseStreamSession = void 0;
...
function shouldStopProseStreamForBlocksProtocol(state) {
    ...
    if (state.mode === 'buffer') {
        ...
        return ((0, message_blocks_util_1.looksLikeBlocksJsonOutput)(remainder) ||
            (0, message_blocks_util_1.looksLikeBlocksJsonOutput)(state.messageText));
    }
    return false;
}
...
function resolveSummarizeLlmProseForStorage(input) {
    let prose = '';
    if (input.proseSession.messageDeltaEmitted &&
        input.proseSession.sanitizedEmitted.trim()) {
        prose = (0, message_blocks_util_1.sanitizeSummarizeUserFacingProse)(input.proseSession.sanitizedEmitted).trim();
    }
    if (!prose) {
        prose = (0, message_blocks_util_1.sanitizeSummarizeUserFacingProse)((0, llm_output_sanitize_util_1.sanitizeLlmFinalOutput)(input.userMarkdown ||
            input.routedMessage ||
            input.rawSource ||
            input.fallbackPlainText)).trim();
    }
    const extracted = (0, message_blocks_util_1.extractProseFromSummarizeLlmRaw)(input.rawSource);
    if (extracted && extracted.length > prose.length) {
        return extracted;
    }
    return prose;
}
exports.resolveSummarizeLlmProseForStorage = resolveSummarizeLlmProseForStorage;
