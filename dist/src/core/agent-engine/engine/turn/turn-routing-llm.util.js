...
const turnRouteSchema = zod_1.z.object({
    ...
    writeChannel: zod_1.z.enum(['none', 'http', 'host']).default('none'),
    hostMutationIntent: zod_1.z.boolean().optional(),
});
...
async function resolveTurnRoute(input) {
    ...
    if (!llmRaw) {
        return (0, turn_routing_util_1.buildTurnRouteFallbackDecision)({
    ...
    const pageContextTaskKind = llmPageContextTaskKind === 'mutation' ? 'none' : llmPageContextTaskKind;
    const llmWriteChannel = (0, parse_llm_write_channel_util_1.resolveLlmWriteChannelFromRaw)({
        route: llmRaw.route,
    ...
    return {
        ...
        llmWriteChannel,
        hostMutationIntent: (0, turn_write_channel_types_1.hostMutationIntentFromWriteChannel)(llmWriteChannel),
    };
}
