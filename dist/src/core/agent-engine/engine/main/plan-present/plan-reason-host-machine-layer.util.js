...
async function runPlanReasonHostMachineLayer(deps, context) {
    ...
    if (fillTools.length === 0) {
        deps.logger.warn(`plan reason host fill skipped: no fill_stream host tools (structured tools use instant dispatch elsewhere) runId=${context.runId}`);
    ...
    deps.logger.warn(`plan reason host fill using observation-only (no DSL stream) runId=${context.runId} hostStepId=${context.hostStepId} fillTools=${fillTools.map((tool) => tool.name).join(',')}`);
