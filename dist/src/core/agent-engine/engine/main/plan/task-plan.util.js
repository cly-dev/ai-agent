...
const plan_step_bind_util_1 = require("./plan-step-bind.util");
...
function buildPlanSnapshot(input) {
    const compiledSteps = input.scopedToolSummaries
        ? (0, plan_step_bind_util_1.compilePlanToolSteps)(input.steps, input.scopedToolSummaries)
        : input.steps;
    return (0, plan_stack_util_1.wrapSnapshotWithPlanStack)(finalizePlanSnapshot(Object.assign(Object.assign({}, input), { steps: compiledSteps })));
}
...
function buildOrchestratedTemplatePlanResult(input) {
    const { hasReadList } = summarizeScopedRoles(input.scopedToolSummaries);
    if (!hasReadList) {
        return null;
    }
    const deliverable = alignDeliverableWithScopedTools(input.deliverable, input.scopedToolSummaries);
...
exports.buildOrchestratedTemplatePlanResult = buildOrchestratedTemplatePlanResult;
...
function planGatherRequiresFullFetch(step) {
    return ((step === null || step === void 0 ? void 0 : step.kind) === 'tool' && step.stopWhen === 'observation_fetch_complete');
}
exports.planGatherRequiresFullFetch = planGatherRequiresFullFetch;
...
function matchingToolNamesForPlanStep(step, scopedTools) {
    var _a;
    if ((_a = step.pinnedToolNames) === null || _a === void 0 ? void 0 : _a.length) {
        const pinned = new Set(step.pinnedToolNames);
        if (scopedTools === null || scopedTools === void 0 ? void 0 : scopedTools.length) {
...
function countConsecutiveLlmRoundsWithoutToolCalls(steps) {
    ...
    for (let index = steps.length - 1; index >= 0; index -= 1) {
        ...
        if ((row === null || row === void 0 ? void 0 : row.type) && REACT_LOOP_SKIP_STEP_TYPES.has(row.type)) {
            continue;
        }
...
function resolveTaskPlanInitialAdvance(input) {
    ...
    if ((0, plan_summarize_gate_util_1.planSummarizeRequiresToolEvidence)(input.plan)) {
        if (!(0, plan_summarize_gate_util_1.planSummarizeHasToolEvidence)({
            plan: input.plan,
            observationBuckets,
            scopedTools: input.scopedTools,
            workflowRun: input.workflowRun,
        ...
        if (!(0, page_context_execution_policy_util_1.planInitialSummarizeReadyOnFresh)({
