...
function normalizeOuterLlmPlanSteps(raw, scopedToolSummaries, availableSkillIds, scopedHostToolNames) {
    var _a, _b;
    const scopedRoles = new Set(scopedToolSummaries.map((tool) => tool.role));
    const toolByName = (0, plan_step_bind_util_1.scopedToolSummaryByName)(scopedToolSummaries);
    const steps = [];
    ...
    for (const row of raw.steps) {
        ...
        if (row.kind === 'tool') {
            const roleRaw = ((_a = row.toolRole) === null || _a === void 0 ? void 0 : _a.trim()) ||
                (scopedTool === null || scopedTool === void 0 ? void 0 : scopedTool.role) ||
                (0, plan_step_bind_util_1.resolveToolRoleForPlanStepId)(id, toolByName);
        ...
        steps.push(Object.assign(Object.assign(Object.assign({ id, phase: row.phase, kind: row.kind, objective }, (toolRole ? { toolRole } : {})), (pinnedToolNames ? { pinnedToolNames } : {})), (row.stopWhen ? { stopWhen: row.stopWhen } : {})));
    }
    return {
...
function normalizeLlmPlanSteps(raw, scopedToolSummaries, scopedHostToolNames) {
    var _a, _b, _c;
    const scopedRoles = new Set(scopedToolSummaries.map((tool) => tool.role));
    const toolByName = (0, plan_step_bind_util_1.scopedToolSummaryByName)(scopedToolSummaries);
    const steps = [];
    ...
    for (const row of raw.steps) {
        ...
        steps.push(Object.assign(Object.assign(Object.assign({ id, phase: row.phase, kind: row.kind, objective }, (toolRole ? { toolRole } : {})), (pinnedToolNames ? { pinnedToolNames } : {})), (stopWhen ? { stopWhen } : {})));
    }
    return {
        steps: steps.length > 0 ? steps : null,
        droppedHostToolStepIds,
...
async function tryBuildTaskPlanViaLlm(input) {
    ...
    const normalized = normalizeLlmPlanSteps(llmRaw, input.planInput.scopedToolSummaries, scopedHostToolNames);
    if (!normalized.steps) {
        return null;
    }
    ...
    const plan = (0, task_plan_util_1.buildPlanSnapshot)({
        source: 'llm',
        userMessage,
        goal: llmRaw.goal.trim() || (0, plan_goal_util_1.resolvePlanGoal)({ userMessage }),
...
async function tryBuildOuterPlanViaLlm(input) {
    ...
    const normalized = normalizeOuterLlmPlanSteps(llmRaw, input.planInput.scopedToolSummaries, availableSkillIds, scopedHostToolNames);
    if (!normalized.steps) {
        return null;
    }
    const userMessage = input.planInput.userMessage.trim();
...
async function resolveOuterPlan(input) {
    ...
    const templateFallback = (0, task_plan_util_1.buildOrchestratedTemplatePlanResult)({
        userMessage,
        scopedToolSummaries: scopedSummaries,
        deliverable: (0, task_plan_util_1.alignDeliverableWithScopedTools)('analysis', scopedSummaries),
    });
    if (templateFallback) {
        return Object.assign(Object.assign({}, templateFallback), { llmFallbackReason: 'outer_plan_llm_failed_template' });
    }
